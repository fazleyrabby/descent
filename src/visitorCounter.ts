 ...
 *
 * Self-hosted (PM2 + SQLite on the VPS / homelab), publicly served through the
 ...
 *
 * API:
 *   GET /api/hit?project=descent&key=visitors  (increment)
 *   GET /api/get?project=descent&key=visitors  (read only)
 */

const VIEWS_BASE = 'https://views.fazleyrabbi.xyz';
const PROJECT = 'descent';
const KEY = 'visitors';
const CACHE_KEY = 'descent:visits';
const SESSION_KEY = 'descent:visitTracked';
...
function buildWidget(count: number): HTMLElement {
  ...
  wrap.title = 'Total expedition visits — live telemetry';
  wrap.innerHTML = `
